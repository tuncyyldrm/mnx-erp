  ...
  return (
    <html lang="tr">
      <body className={`${inter.className} bg-slate-50 text-slate-900 antialiased pb-24 md:pb-0`}>
        ...
        {/* --- MOBİL ALT NAVİGASYON (Sadece Mobilde) --- */}
        <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-2xl border-t border-slate-100 px-6 py-2 flex justify-between items-center z-[120] md:hidden shadow-[0_-10px_40px_-10px_rgba(15,23,42,0.15)] rounded-t-[32px]">
        ...
        {/* ANA İÇERİK */}
        <main className="min-h-screen max-w-[1600px] mx-auto p-4 md:p-8">
          {children}
        </main>

        {/* MASAÜSTÜ FOOTER */}
        <footer className="hidden md:block border-t border-slate-100 bg-white py-12">
...
function ActionCard({ href, icon, title, desc, color }: any) {
  ...
  return (
    <Link href={href} className="m-card !rounded-[24px] !p-4 flex items-center justify-between group active:scale-[0.98] transition-all">
      <div className="flex items-center gap-4">
...
// SVG İkonlar (Stroke width 2.5 ile daha endüstriyel)
