...
export default function UrunHareketPage() {
  ...
  const fetchMovements = useCallback(async () => {
    ...
    try {
      ...
      // 2. Hareketleri çek (İlişkili Tablo: transactions ve contacts)
      ...
      console.error("Memonex Hareket Yükleme Hatası:", err);
    } finally {
      setLoading(false);
    }
  }, [id]);
  ...
  const totalIn = movements
    .filter(m => ['purchase', 'return_in'].includes(m.transactions?.type))
    .reduce((acc, curr) => acc + curr.quantity, 0);

  const totalOut = movements
    .filter(m => ['sale', 'return_out'].includes(m.transactions?.type))
    .reduce((acc, curr) => acc + curr.quantity, 0);
  ...
  return (
    <div className="p-4 md:p-12 max-w-[1500px] mx-auto text-slate-900 bg-[#F8FAFC] min-h-screen font-sans">
      ...
      <div className="flex flex-col gap-10 mb-12">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-10 bg-white p-8 md:p-12 rounded-[48px] border-2 border-slate-100 shadow-xl shadow-slate-200/40">
          <div className="flex items-center gap-8 md:gap-12 w-full lg:w-auto">
            {/* Ürün Görseli / SKU Avatar */}
            <div className="w-28 h-28 md:w-36 md:h-36 shrink-0 bg-slate-100 rounded-[40px] overflow-hidden border-4 border-white shadow-2xl relative group">
              {product.image_url ? (
            ...
            <div className="space-y-4">
              ...
              <h1 className="text-1xl md:text-1xl font-black italic uppercase tracking-tighter leading-[0.9] text-slate-900">
                {product.name}
              </h1>
              <p className="text-slate-400 font-bold uppercase italic text-sm tracking-tight">{product.brand || 'Markasız Ürün'}</p>
            </div>
          </div>
...
function DesktopRow({ m }: any) {
  ...
  const transactionId = m.transactions?.id;
  
  // Belge İzleme Linki Mantığı
  const viewLink = ['sale', 'return_in'].includes(type) 
    ? `/satis/izle/${transactionId}` 
    : `/alis/izle/${transactionId}`;

  const typeMap: any = {
    sale: { label: 'SATIŞ', color: 'bg-red-50 text-red-600' },
    purchase: { label: 'ALIM', color: 'bg-emerald-50 text-emerald-600' },
    return_in: { label: 'MÜŞT. İADE', color: 'bg-blue-50 text-blue-600' },
    return_out: { label: 'TED. İADE', color: 'bg-amber-50 text-amber-600' }
  };

  const currentType = typeMap[type] || { label: 'DİĞER', color: 'bg-slate-50 text-slate-400' };
    ...
    <tr className="hover:bg-slate-50/80 transition-all group">
      ...
      <td className="p-12">
        <div className="flex items-center gap-6">
          ...
          <div>
            <div className="font-black uppercase text-slate-900 tracking-tight text-base mb-1 group-hover:text-blue-600 transition-colors italic">
            ...
            <div className="flex items-center gap-4">
               ...
               <Link 
                 href={viewLink}
                 className="flex items-center gap-1.5 text-[9px] font-black text-blue-500 hover:text-blue-700 transition-all uppercase tracking-widest border-b-2 border-blue-50 hover:border-blue-500 pb-0.5"
               >
                 <ExternalLink size={10} /> BELGE İZLE
               </Link>
            </div>
          </div>
        </div>
      ...
      <td className="p-12 text-right font-bold text-slate-400 text-sm italic">
        {m.unit_price?.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} TL
      </td>
      <td className="p-12 text-right">
        <div className="text-2xl font-black tracking-tighter text-slate-900 group-hover:scale-110 transition-transform origin-right italic">
          {m.line_total?.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} <small className="text-[12px] not-italic opacity-30">TL</small>
        </div>
      </td>
    </tr>
  );
...
function MobileRow({ m }: any) {
  ...
  const isOut = ['sale', 'return_out'].includes(type);
  const transactionId = m.transactions?.id;
  const viewLink = ['sale', 'return_in'].includes(type) ? `/satis/izle/${transactionId}` : `/alis/izle/${transactionId}`;
  ...
  return (
    <div className="p-8 space-y-6 bg-white">
      <div className="flex justify-between items-start">
        <div className="flex gap-5">
          ...
          <div className="space-y-2">
            <p className="font-black uppercase text-slate-900 text-base leading-none italic">{m.transactions?.contacts?.name || 'GENEL HAREKET'}</p>
            <div className="flex items-center gap-3">
               <p className="text-[10px] font-bold text-slate-400 uppercase italic">
                 {new Date(m.created_at).toLocaleDateString('tr-TR')}
               </p>
               <Link href={viewLink} className="bg-blue-50 text-blue-600 text-[9px] font-black px-3 py-1 rounded-lg uppercase tracking-tighter">
                 BELGEYİ GÖR
               </Link>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 gap-4 bg-slate-50 p-6 rounded-[24px] border border-slate-100">
        ...
        <div className="flex flex-col text-right border-l border-slate-200 pl-4">
          ...
          <span className="font-black text-lg text-slate-900 italic">{m.line_total?.toLocaleString('tr-TR')} TL</span>
        </div>
      </div>
    </div>
  );
